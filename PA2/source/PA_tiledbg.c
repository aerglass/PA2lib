...
void PA_InitBgBuffers(void) {
	...
	for (n = 0; n < PA_SLOTS_TBG; n ++) {	// Inicializa todos los slots
		...
		PA_TILEDBG[n].tilesize = 0;			// Tama�o del Tileset
		PA_TILEDBG[n].mapsize = 0;			// Tama�o del Mapa
		PA_TILEDBG[n].palsize = 0;			// Tama�o de la Paleta
...
void PA_LoadBg(const char* file, const char* name, u16 width, u16 height) {
	...
	if (file_id) {	// Si el archivo existe...
		// Obten el tama�o del archivo
		fseek(file_id, 0, SEEK_END);
		PA_TILEDBG[slot].mapsize = ((((ftell(file_id) - 1) >> 11) + 1) << 11);	// Ajusta el tama�o a bloques de 2kb
		rewind(file_id);
		// Reserva el espacio en RAM
...
void PA_LoadTilesForBg(const char* file, const char* name, u16 width, u16 height, u16 tile_start, u16 tile_end) {
	...
	if (file_id) {	// Si el archivo existe...
		...
		PA_TILEDBG[slot].tilesize = (((tile_end - tile_start) + 1) << 6);	// n� de tiles x 64 bytes
...
void PA_UnloadBg(const char* name) {
	...
	PA_TILEDBG[slot].tilesize = 0;					// Tama�o del Tileset
	PA_TILEDBG[slot].mapsize = 0;					// Tama�o del Mapa
	PA_TILEDBG[slot].palsize = 0;					// Tama�o de la Paleta
...
void PA_CreateBg(u8 screen, u8 layer, const char* name) {
	...
	// Calcula el tipo y tama�o del mapa si es IPAinito ( >512 )
	// ( >512 x n )
	...
	PA_EasyBgScrollXY(screen, layer, 0, 0);
