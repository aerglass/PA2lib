...
void PA_InitSpriteBuffers(void) {
	...
	for (n = 0; n < PA_SLOTS_SPR256GFX; n ++) {
		...
		PA_SPR256GFX[n].size = 0;				// Tamaño (en bytes) del grafico (GFX)
	...
	for (n = 0; n < PA_SLOTS_SPR256PAL; n ++) {
		...
		PA_SPR256PAL[n].size = 0;			// Tamaño (en bytes) de la paleta (PAL)
...
void PA_InitSpriteSys(int screen, ...) {
	...
	for (n = 0; n < 128; n ++) {	// 128 sprites
		...
		PA_SPR256VRAM[screen][n].size = 0;				// Tamaño (en bytes) del Gfx
		...
		PA_SPR256VRAM[screen][n].framesize = 0;			// Tamaño del frame (en bytes)
		...
		PA_SPRITEOAM[screen][n].size = SpriteSize_8x8;					// Tamaño del Sprite (macro) (8x8 por defecto)
		...
		PA_SPRITEOAM[screen][n].framesize = 0;			// Tamaño del frame (en bytes)
	...
	for (n = 0; n < 128; n ++) {
		...
		PA_SPRVRAM[screen].size[n] = 0;		// Tamaño del bloque libre para reusar
	}
...
void PA_UnloadSpriteGfx(u16 id) {
	...
	PA_SPR256GFX[id].size = 0;				// Tamaño (en bytes) del grafico (GFX)
...
void PA_UnloadSpritePal(u8 id) {
	...
	PA_SPR256PAL[id].size = 0;			// Tamaño (en bytes) de la paleta (PAL)
...
void PA_VramSpriteGfx(u8 screen, u16 ram, u16 vram, bool keepframes) {
	...
	if (PA_SPRVRAM[screen].deleted > 0) {
		...
		for (n = 0; n < PA_SPRVRAM[screen].deleted; n ++) {
			if (PA_SPRVRAM[screen].size[n] == gfxsize) {		// Si el bloque tiene el tamaño suficiente
				id = n;		// Guarda la Id
	...
	if (id != 255) {
		...
		if (gfxsize < PA_SPRVRAM[screen].size[id]) {
			...
			PA_SPRVRAM[screen].size[id] = size;				// Nuevo tamaño
		...
		if (organize) {
			...
			(id != last_reuse)	// Y no es la ultima posicion
				...
				PA_SPRVRAM[screen].size[id] = PA_SPRVRAM[screen].size[last_reuse];		// Nuevo tamaño
			}
	...
	PA_SPR256VRAM[screen][vram].size = gfxsize;						// Tamaño en bytes de los datos copiados
...
void PA_FreeSpriteGfx(u8 screen, u16 id) {
	...
	PA_SPR256VRAM[screen][id].size = 0;			// Tamaño en bytes
	...
	PA_SPR256VRAM[screen][id].framesize = 0;	// Tamaño del frame (en bytes)
...
void PA_VramSpriteGfxDefrag(u8 screen) {
	...
	for (n = 0; n < 128; n ++) {
		...
		if (PA_SPR256VRAM[screen][n].inuse) {
			...
			size[n] = PA_SPR256VRAM[screen][n].size;		// Almacena el tamaño
...
void PA_CreateSprite(u8 screen, u8 id, u16 gfx, u8 pal, s16 x, s16 y) {
	...
	// IPAorma al array de OAM del tamaño
	if ((PA_SPR256VRAM[screen][gfx].width == 8) && (PA_SPR256VRAM[screen][gfx].height == 8)) {	// 8x8
		if (PA_SPRVRAM[screen].max != 131072) {		// En modo 1D_128, este tamaño es ilegal
	...
	// IPAorma al array de OAM del tamaño del frame del Sprite (en bytes)
...
void PA_DeleteSprite(u8 screen, u8 id) {
	...
	PA_SPRITEOAM[screen][id].size = SpriteSize_8x8;					// Tamaño del Sprite (macro) (8x8 por defecto)
	...
	PA_SPRITEOAM[screen][id].framesize = 0;			// Tamaño del frame (en bytes)
	...
	if (screen == 0) {
		...
		for (n = 0; n < 128; n ++) {
			oamSet(&oamMain,							// OAM pantalla superior (Main, 0)
				...
				PA_SPRITEOAM[screen][n].size,			// Tamaño del Sprite (macro)
