...
void PA_LoadBMP(const char* file, u8 slot) {
	...
	sprintf(filename, "%s/%s.bmp", PA_ROOTFOLDER, file);
	...
	if (file_id) {	// Si el archivo existe...
		...
		if (strcmp(magic_id, "BM") == 0) {
			...
			if (buffer == NULL) PA_Error(102, NULL, bmp_header.raw_size);
	...
	if (PA_BG16B[slot].buffer == NULL) PA_Error(102, NULL, size);
	...
	switch (bmp_header.bpp) {
		...
		case 8:		// 8 bits por pixel
			...
			if (palette == NULL) PA_Error(102, NULL, (colors << 2));
			// Abre de nuevo el archivo y carga la paleta
			file_id = fopen(filename, "rb");
			if (!file_id) PA_Error(101, filename, 0);
			...
			for (y = 0; y < bmp_header.bmp_height; y ++) {
				for (x = 0; x < bmp_header.bmp_width; x ++) {
					...
					PA_BG16B[slot].buffer[offset] = pixel;
					// Siguiente pixel
					idx ++;
				}
	...
	PA_BG16B[slot].width = bmp_header.bmp_width;		// Ancho del fondo
	PA_BG16B[slot].height = bmp_header.bmp_height;		// Altura del fondo
